...
public class TopSite {
	...
	public static void main(String[] args) {
		...
		String map = "function() { emit(this.site_name, {key: this.site_name, count: 1});}";
		String reduce = "function(key, values) { total = 0; for (var i in values) "
				+ "{ total += values[i].count; } return {key: key, count: total};}";
		MapReduceCommand cmd = new MapReduceCommand(collection, map, reduce, "topSite", MapReduceCommand.OutputType.REPLACE, null);
		MapReduceOutput out = collection.mapReduce(cmd);
		collection = db.getCollection("topSite");
