...
public class Part1 {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			FileStatus[] inputFiles = input.listStatus(inputDir);
			FSDataOutputStream out = hdfs.create(hdfsFile);
			for (int i=0; i<inputFiles.length; i++) {
				FSDataInputStream in = input.open(inputFiles[i].getPath());
	    ...
	    FileOutputFormat.setOutputPath(job, new Path("final" + args[1]));
